...
2. Browser is redirected to `https://matrix.inquiry.institute/_matrix/client/v3/login/sso/redirect/supabase?redirectUrl=<atrium-url>`.
...
- **IdP id on Synapse**: `supabase` (path segment in SSO redirect URL; must match Synapse’s `idp_id`).
